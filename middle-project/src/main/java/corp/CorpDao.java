...
public class CorpDao {
	...
	public void insertApi(Corp o, int num) {
		...
		try {
			...
			System.out.println(cnt2 + "줄 기업추가");
	...
	public void insertNew(Corp o) {
		Connection conn = db.conn();
		String sql = "insert into corp values(seq_corp.nextval,?,?,?,?)";
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);					
			pstmt.setString(1, o.getCorpid());
			pstmt.setString(2, o.getCorp_nm());
			pstmt.setString(3, o.getCorp_addr());
			pstmt.setString(4, o.getBusi_no());

			int cnt = pstmt.executeUpdate();
			System.out.println(cnt + "줄 기업추가");
...
//	}
	...
	public Corp selectByCorpId(String corpid) {
		Connection conn = db.conn();
		String sql = "select * from corp where corpid=?";
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);
